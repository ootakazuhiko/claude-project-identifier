...
echo ""
echo -e "${YELLOW}${BOLD}🎯 プロンプト統合の設定方法:${NC}"
echo ""
echo -e "${PURPLE}Claude Code環境では:${NC}"
echo "1. プロジェクト情報を表示:"
echo -e "   ${GREEN}./init-project.sh${NC}"
echo ""
echo "2. 環境変数を設定（オプション）:"
echo -e "   ${GREEN}source .claude-prompt.sh${NC}"
echo -e "   ${GREEN}echo \"現在のプロジェクト: \$CLAUDE_PROJECT_NAME\"${NC}"
echo ""
echo -e "${PURPLE}ローカルターミナルでは:${NC}"
echo "1. ホームディレクトリにコピー:"
...
echo ""
echo "3. シェル再読み込み:"
echo -e "   ${GREEN}source ~/.bashrc${NC}  # または ~/.zshrc"
echo ""
echo -e "${YELLOW}注意: Claude Codeはブラウザ環境のため、ターミナルタイトルバーの変更はローカル環境でのみ有効です${NC}"

echo -e "${PURPLE}今すぐテスト:${NC}"
echo -e "   ${GREEN}./init-project.sh${NC}  # プロジェクト情報表示"
echo -e "   ${GREEN}source .claude-prompt.sh${NC}  # 環境変数設定"
if grep -q "info:" Makefile 2>/dev/null; then
    echo -e "   ${GREEN}make info${NC}  # Makefile経由での表示"
fi
echo ""
